...
//FOR SEARCHING FUNCTIONS ###################################################################################
  ...
  public function actionStockdelete(){
   ...
   //         return $this->redirect(Url::to(['/admin/default/401']));
   //      }else{
   //          echo json_encode(array('itemcount' => $return['itemcount'],'grandtotal'=>$return['grandtotal'],'totalcbm'=>$return['totalcbm'],'totaltonnage'=>$return['totaltonnage'],'istransposted'=>$return['istransposted']));
   //      }         
   //        } catch (ErrorException $e) {
   //      echo $e;
   //      return 0;
   ...
   public function actionComparestocklines(){     
    ...
    //   $items = Yii::$app->backend->getItemsAvailableForVoid($doc,$params['q'],$params['searchstring']);
    ...
    /*public function actionSavesonote(){
      ...
      $qry="insert into sbc_so_notes (trno,station,serialno,rem,others,line) 
      values(".$params['trno'].",'".$params['sonotestation']."','".$params['sonoteserial']."',
      '".$params['sonoteremarks']."','".$params['sonoteothers']."',".$params['line'].");";
    ...
    public function actionRetrievesonote(){
      ...
      $qry="select station,serialno,rem,others,line from sbc_so_notes where trno = ".$trno."";
      $sonotes = Yii::$app->sbccommon->openTable($qry);
      echo json_encode(array('sonotes'=>$sonotes));
    }//end if
    ...
    public function actionDeletesonote(){
      ...
      if($status){
        ...
        $msg = "Delete note failed! Please try again!";
      }//end if status
