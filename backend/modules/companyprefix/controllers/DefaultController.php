...
class DefaultController extends Controller{
    ...
    public function actionSaveprefix(){
    	...
    	if(empty($line)){
    		$qry = "insert into company_prefixes (companyname,availprefs,companyalias,company_add,company_tel,cgrp) 
            values('".$company."','".$prefixes."','".$alias."','".$add."','".$tel."','".$cg."')";
    	}else{
    		$qry = "update company_prefixes set companyname = '".$company."', availprefs = '".$prefixes."',companyalias = '".$alias."',
            company_add = '".$add."',company_tel = '".$tel."',cgrp = '".$cg."' where line = '".$line."'";
    	}//end if
    ...
    public function actionGetcompanyprefixgroup(){
        $data = Yii::$app->backend->getcompanyprefixgroup();
