...
Yii::$app->reporter->col('SALES SUMMARY PER VAT TYPE (SUMMARIZED)',null,null,false,'1px solid ','','c','Helvetica','13','','','').'<br />';
...
Yii::$app->reporter->begintable('800');
       ...
       Yii::$app->reporter->col('RETURNS','100',null,false,'1px solid ','TB','R','Helvetica','12','B','','');
Yii::$app->reporter->endtable();
...
for($i=0;$i<count($data);$i++){
    ...
    $qryreturn = "select ifnull(sum(totalreturns),0) as totalreturn from (
    select ifnull(sum(rstock.ext),0) as totalreturns from lahead as rhead 
    left join lastock as rstock on rstock.trno = rhead.trno 
    left join client as rsupp on rsupp.client = rhead.client 
    where rhead.doc = 'CM' 
    and rsupp.client =  '".$data[$i]['client']."' 
    and rhead.dateid >='".$start."' and rhead.dateid <='".$end."'
    group by rhead.trno 
    UNION ALL
    select ifnull(sum(rstock.ext),0) as totalreturns from glhead as rhead 
    left join glstock as rstock on rstock.trno = rhead.trno 
    left join client as rsupp on rsupp.clientid = rhead.clientid 
    where rhead.doc = 'CM' 
    and rsupp.client = '".$data[$i]['client']."' 
    and rhead.dateid >='".$start."' and rhead.dateid <='".$end."' 
    group by rhead.trno
    ...
    $returns = Yii::$app->sbccommon->datareader($qryreturn);

    Yii::$app->reporter->addline();
    Yii::$app->reporter->startrow();
       Yii::$app->reporter->col($data[$i]['client'],'100',null,false,'1px dashed ','B','L','Helvetica','11','','','');
       Yii::$app->reporter->col($data[$i]['supplier'],'200',null,false,'1px dashed ','B','L','Helvetica','11','','','');
       ...
       Yii::$app->reporter->col(number_format($data[$i]['vatamt'],Yii::$app->systemsettings->setDecimaldisplay('currency')),'100',null,false,'1px dashed ','B','R','Helvetica','11','','','');
       Yii::$app->reporter->col(number_format($data[$i]['vatex'],Yii::$app->systemsettings->setDecimaldisplay('currency')),'100',null,false,'1px dashed ','B','R','Helvetica','11','','','');
        Yii::$app->reporter->col(number_format($returns,Yii::$app->systemsettings->setDecimaldisplay('currency')),'100',null,false,'1px dashed ','B','R','Helvetica','11','','',''); 
       ...
       /* if(Yii::$app->reporter->linecounter==$page){
        ...
        Yii::$app->reporter->begintable('800');
               Yii::$app->reporter->startrow();
               Yii::$app->reporter->col('CODE','100',null,false,'1px solid ','TB','L','Helvetica','12','B','','');
               Yii::$app->reporter->col('CUST. NAME','300',null,false,'1px solid ','TB','L','Helvetica','12','B','','');
               Yii::$app->reporter->col('TIN','100',null,false,'1px solid ','TB','L','Helvetica','12','B','','');
               Yii::$app->reporter->col('AMOUNT','100',null,false,'1px solid ','TB','R','Helvetica','12','B','','');
               Yii::$app->reporter->col('VAT','100',null,false,'1px solid ','TB','R','Helvetica','12','B','','');
               Yii::$app->reporter->col('VATEX','100',null,false,'1px solid ','TB','R','Helvetica','12','B','','');
...
Yii::$app->reporter->startrow();
       ...
       Yii::$app->reporter->col(number_format($totalreturns,Yii::$app->systemsettings->setDecimaldisplay('currency')),'100',null,false,'2px solid ','TB','R','Helvetica','10','B','','');
Yii::$app->reporter->endrow();
