...
$this->title = 'Invoice without Batch';
?>
...
for($i=0;$i<count($data);$i++){
        ...
        Yii::$app->reporter->col(number_format($data[$i]['isqty'],Yii::$app->systemsettings->setDecimaldisplay('currency')),'50',null,false,'1px dotted ','','C','Helvetica','12','','','0px');
        Yii::$app->reporter->col($data[$i]['uom'],'50',null,false,'1px dotted ','','C','Helvetica','12','','','0px');
        Yii::$app->reporter->col($data[$i]['itemname'],'350',null,false,'1px dotted ','','L','Helvetica','12','','','0px');
        Yii::$app->reporter->col(number_format($data[$i]['isamt'],Yii::$app->systemsettings->setDecimaldisplay('currency')),'100',null,false,'1px dotted ','','R','Helvetica','12','','','0px');
        Yii::$app->reporter->col($data[$i]['disc'],'50',null,false,'1px dotted ','','R','Helvetica','12','','','2px');
        Yii::$app->reporter->col(number_format($data[$i]['amt'],Yii::$app->systemsettings->setDecimaldisplay('currency')),'100',null,false,'1px dotted ','','R','Helvetica','12','','','0px');
        Yii::$app->reporter->col(number_format($data[$i]['ext'],Yii::$app->systemsettings->setDecimaldisplay('currency')),'100',null,false,'1px dotted ','','R','Helvetica','12','','','0px');
        $totalext = $totalext + $data[$i]['ext'];
        $rowcounter += 1;
}   
      
    if($rowcounter != 26){
        $rowadd = 26 - $rowcounter;
        for ($i=0; $i < $rowadd; $i++) { 
            Yii::$app->reporter->startrow();
            Yii::$app->reporter->addline();
    ...
    Yii::$app->reporter->begintable('800');
        Yii::$app->reporter->startrow();
        Yii::$app->reporter->col('NOTE : ','70',null,false,'1px dotted ','','L','Helvetica','12','B','','');
        Yii::$app->reporter->col((isset($data[0]['rem'])? $data[0]['rem']:''),'720',null,false,'1px dotted ','','L','Helvetica','12','I','','');
    ...
    echo '<br/>';
        ...
        Yii::$app->reporter->col('','400',null,false,'1px dotted ','B','L','Helvetica','12','B','','');
        Yii::$app->reporter->endrow();
    Yii::$app->reporter->endtable();
