...
namespace backend\modules\TR\controllers;
...
//THESE ACCESS ARRAY IS USED TO DETERMINE ACCESS INDEX ON ATTRIBUTES
   public $access = array('view' => 785,'edit' => 786,'new' =>787,
        'save' => 788,'change' => 789,'delete' => 790,'print' => 791,
        'lock' => 792,'unlock' => 793,'post' => 794,'unpost' => 795,'changeamount'=>842,
        'clickadditem'=>839,'clickedititem'=>840,'clickdeleteitem'=>841);
...
//FOR BUTTON FUNCTIONS ###################################################################################
  ...
  public function actionGetroutes() {
    Yii::$app->backend->AjaxVerification($this);
    return Yii::$app->automator->automateRoutelookup($this);
  }

  //UPDATES FOR COPY FROM
  public function actionGetcopydocuments(){
    Yii::$app->backend->AjaxVerification($this);
    $doc = Yii::$app->backend->sanitize($_GET['doc'],'DEFAULT');
    $q = Yii::$app->backend->sanitize($_GET['q'],'DEFAULT');
    $copydocuments = Yii::$app->backend->getCopyDocuments($doc,$q);

    Yii::$app->response->format = Response::FORMAT_JSON;
    return ['copydocuments'=>$copydocuments];
   //echo json_encode(array('copydocuments'=>$copydocuments));
  }//end action copy documents

  public function actionItemcopyfrom(){
    Yii::$app->backend->AjaxVerification($this);
    $params = Yii::$app->backend->sanitize($_GET,'ARRAY');
    $copieditems = Yii::$app->backend->copyStockItemsFrom($params['doc'],$params['q']);

    Yii::$app->response->format = Response::FORMAT_JSON;
    return ['primarydata'=>$copieditems];
   //echo json_encode(array('primarydata'=>$copieditems));
  }//end action item copy from
  ...
  //jly gg d7 stock
   public function actionBuildstockview(){
      ...
      return Yii::$app->automator->generateTRStockview($sql,$type);
   }//end function

      //jly GG d7 docnolookup
    ...
    //jly gg d2 balance
    public function actionGetitembalancegv(){
      Yii::$app->backend->AjaxVerification($this);
      $params = $_POST;
      // var_dump($params);
      $params['controller'] = $this;
      return Yii::$app->automator->automateItembalGV($params);
    }
