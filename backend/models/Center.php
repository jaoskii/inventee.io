...
class Center extends Model
{
    ...
    public function rules()
    {
        return array(
            ...
            array('line, code, name,mainmodule,station,selectmodule', 'safe', 'on'=>'search'),
        );
    }
    ...
    public function attributeLabels()
    {
        return array(
          ...
          public static function getbranches_() {
            ...
            $data=Yii::$app->sbccommon->opentable("select concat(line, '/', code) as id,code,name,address,tel,station,line, warehouse from center");
            return $data;
            if(!empty($data))
                {
                    $centers=array("");
                    ...
                    {
                    $center=array('id'=>strtoupper($data_['line']."/".$data_['code']),'code'=>$data_['code'],'name'=>$data_['name'],'line'=>$data_['line'],'address'=>$data_['address'],'tel'=>$data_['tel'],'station'=>$data_['station'],'warehouse'=>$data_['warehouse']);
        ...
        public static function getbranches_line($line) {
            $data=Yii::$app->sbccommon->opentable("select center.line, code, center.name, address, tel,station, warehouse from center where line = '$line'");
            if(!empty($data))
                {
                    $centers=array("");
                    foreach($data as $key => $data_)
        ...
        {
            $data=Yii::$app->sbccommon->opentable("select center.line, code, center.name, address, tel, warehouse,sellingprice,
                                                    commission,icommission from center
                                                    left join centeraccess on centeraccess.center=center.code
                                                    where md5(userid)='$userid' order by center.name");
            if(!empty($data))
                {
                    $centers=array();
                    foreach($data as $key => $data_)
